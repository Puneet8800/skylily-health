  ...
  },
  {
    ...
    check: () => {
      try {
        ...
        if (status.includes('OK')) return { ok: true, detail: 'connected' };
  ...
  },
  {
    ...
    check: () => {
      try {
        ...
        if (status.includes('stopped')) return { ok: false, detail: 'stopped' };
  ...
  },
  {
    ...
    check: () => {
      try {
        ...
        if (pct > 90) return { ok: false, detail: `${parts[4]} used - CRITICAL` };
        if (pct > 80) return { ok: true, detail: `${parts[4]} used - WARNING` };
  ...
  },
  {
    ...
    check: () => {
      try {
        ...
        if (mbFree < 500) return { ok: false, detail: `${mbFree.toFixed(0)}MB free - LOW` };
